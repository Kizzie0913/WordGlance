// fix_md5.js - 替换 photoTranslate.js 中的 MD5 函数为正确版本
const fs = require('fs');
...
function md5(input) {
  ...
  function md5_cmn(q, a, b, x, s, t) { return safe_add(bit_rol(safe_add(safe_add(a, q), safe_add(x, t)), b), s); }
  function md5_ff(a, b, c, d, x, s, t) { return md5_cmn((b & c) | ((~b) & d), a, b, x, s, t); }
  function md5_gg(a, b, c, d, x, s, t) { return md5_cmn((b & d) | (c & (~d)), a, b, x, s, t); }
  function md5_hh(a, b, c, d, x, s, t) { return md5_cmn(b ^ c ^ d, a, b, x, s, t); }
  function md5_ii(a, b, c, d, x, s, t) { return md5_cmn(c ^ (b | (~d)), a, b, x, s, t); }
  function md5_md5(x, len) {
  ...
  function md5_binl2rstr(input) { var i, output = ''; for (i = 0; i < input.length * 32; i += 8) output += String.fromCharCode((input[i >> 5] >>> (i % 32)) & 0xFF); return output; }
  function md5_rstr2binl(input) { var i, output = []; for (i = 0; i < input.length * 8; i += 32) output[i >> 5] = 0; for (i = 0; i < input.length * 8; i += 8) output[i >> 5] |= (input.charCodeAt(i / 8) & 0xFF) << (i % 32); return output; }
  function md5_rstr_md5(s) { return md5_binl2rstr(md5_md5(md5_rstr2binl(s), s.length * 8)); }
  function rstr2hex(input) { var hex_tab = "0123456789abcdef"; var output = ""; var x; for (var i = 0; i < input.length; i++) { x = input.charCodeAt(i); output += hex_tab.charAt((x >>> 4) & 0x0F) + hex_tab.charAt(x & 0x0F); } return output; }
  return rstr2hex(md5_rstr_md5(input));
}`;

// 找到旧 MD5 函数的位置并替换
const md5Start = content.indexOf('// ========== MD5函数');
const baiduStart = content.indexOf('// ========== 百度翻译API配置');

if (md5Start > 0 && baiduStart > md5Start) {
  const oldMd5Len = baiduStart - md5Start;
  console.log('旧MD5函数长度:', oldMd5Len);
  content = content.substring(0, md5Start) + newMD5 + '\n\n' + content.substring(baiduStart);
  fs.writeFileSync(path, content, 'utf8');
  console.log('✅ MD5函数已替换');
  ...
  try {
    ...
    console.log('❌ JS语法错误:', e.message.substring(0, 100));
  }
} else {
  console.log('❌ 未找到MD5函数位置');
  console.log('md5Start:', md5Start, 'baiduStart:', baiduStart);
}
